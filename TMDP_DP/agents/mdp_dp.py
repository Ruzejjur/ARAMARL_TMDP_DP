...
class _BaseLevelK_MDP_DP_Agent(LearningAgent):
    """
    ...
    def _setup_value_function(self, initial_V_value: float) -> ValueFunction:
        """
        Initializes the value function V(s).
        ...
        for s in tqdm(range(self.n_states), desc=desc_str):
            ...
            for a_self_exec in range(self.num_self_actions):
                for a_opp_exec in range(self.num_opponent_actions):
                    ...
                    s_prime, rewards_vec, _, _, _ = self.env_snapshot.step(action_pair)
...
class LevelK_MDP_DP_Agent_Stationary(_BaseLevelK_MDP_DP_Agent):
    """
    A Level-K DP agent that assumes a stationary environment transition model.

    "Stationary" means the probabilities of an agent's action succeeding or failing
    are fixed and known. This agent pre-computes this state-independent
    transition tensor once. It then dynamically combines this fixed tensor with
    its learned, non-stationary estimate of the opponent's policy to form a
    complete, state-dependent world model for decision-making.

    It models its opponent recursively:
    - A Level-1 agent models its opponent as Level-0 (learning a random policy).
    - A Level-k (k>1) agent models its opponent as a Level-(k-1) agent.
    ...
    def optim_act(self, obs: State) -> Action:
        """
        Selects the optimal action using the Bellman equation and vectorized operations.
        
        Calculates Q(s,a) for all actions 'a' by marginalizing over the opponent's
        predicted policy and all possible stochastic outcomes, then chooses the best one.
        """
        ...
        # Combine the fixed transition tensor with the learned opponent policy for this state.
        ...
        max_indices = np.flatnonzero(q_values_for_actions == np.max(q_values_for_actions))
    ...
    def update(self, obs: State, actions: tuple[Action, Action], new_obs: State, rewards = None):
        """
        Updates the agent's value function V(s) and its internal opponent model.
        
        Performs a single step of value iteration for the observed state `obs` and
        updates the opponent model based on their observed action.
        
        Args:
            obs: The state before the actions were taken.
            actions: The intended actions of (self, opponent).
            new_obs: The resulting state.
        """
        ...
        # --- Vectorized V(obs) Update ---
        
        # Get outcomes from the lookup tables for the current state.
        rewards_executed = self.r_lookup[obs, :, :]
        s_primes_executed = self.s_prime_lookup[obs, :, :]
        
        # Calculate the transition model using the *updated* opponent policy estimate.
        state_dependent_transition_tensor = self._calculate_state_dependent_transition_tensor(obs)
    ...
    def _calculate_state_dependent_transition_tensor(self, obs): 
        """
        Creates the state-dependent transition model.

        It marginalizes out the opponent's intended action by weighting the
        state-independent transition tensor with the opponent's learned policy
        for the given state `obs`.

        The calculation is: P(a_s_exec, a_o_exec | s, a_s_int) = 
        Σ_{a_o_int} [ P(a_s_exec, a_o_exec | a_s_int, a_o_int) * p(a_o_int | s) ]
        
        Returns:
            A 3D np.ndarray with dimensions: (intended_self, executed_self, executed_opp)
        """
        
        # Get opponent's policy in obs
        ...
        # 'ijlk, j -> ilk' sums over the opponent's intended action 'j'.
        state_dependent_transition_tensor = np.einsum(
            'ijlk, j -> ilk', 
            self.prob_exec_tensor, 
            opponent_policy, 
            optimize=True
        )
...
class LevelK_MDP_DP_Agent_NonStationary(LevelK_MDP_DP_Agent_Stationary):
    """
    A Level-K DP agent for non-stationary environments.

    This agent extends the stationary agent to handle cases where the
    environment's action execution probabilities (e.g., action success rate)
    can change during the simulation. It adapts by recalculating its
    transition model before every action.
    ...
    def recalculate_transition_model(self, env):
        ...
        probabilities from the provided environment instance.
        """
        ...
        if self.k > 1 and self.opponent:
            ...
            assert self.opponent is not None, "Opponent model must be set for Level > 1 agents."
            opponent_model = cast(LevelK_MDP_DP_Agent_NonStationary, self.opponent)
            
            opponent_model.recalculate_transition_model(env)

    def act(self, obs: State, env=None) -> Action:
        """
        Selects an action after updating the model to reflect current environment dynamics.
        """
...
class LevelK_MDP_DP_Agent_Dynamic(LevelK_MDP_DP_Agent_Stationary):
    """
    A Level-K DP agent that learns the environment's transition model online.

    This agent does not assume a fixed or known transition model. Instead, it
    learns a state-dependent model P(executed | intended, state) by maintaining
    Dirichlet counts for observed transition outcomes. This makes it suitable
    for environments with unknown or complex state-dependent stochasticity.
    ...
    def __init__(self, k: int, action_space: np.ndarray, opponent_action_space: np.ndarray, lower_level_k_epsilon: float,
        ...
        # 5D tensor to store transition counts (Dirichlet priors).
    ...
    def _get_probabilities_for_state(self, obs: State) -> np.ndarray:
        """
        Calculates P(executed | intended) for a given state using learned counts.
        
        Normalizes the Dirichlet counts for the specified state `obs` to produce
        a state-dependent probability tensor.

        Returns:
            A 4D np.ndarray for the specific state `obs`, with dimensions:
            (intended_self, intended_opp, executed_self, executed_opp)
        """
    ...
    def optim_act(self, obs: State) -> Action:
        """
        Selects the optimal action using the dynamically learned transition model.
        """
    ...
    def update(self, obs: State, actions: tuple[Action, Action], new_obs: State, rewards = None):
        """
        Updates the transition model, the opponent model, and the value function.
